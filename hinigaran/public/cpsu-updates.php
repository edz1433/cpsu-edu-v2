			...
			<div class='row' >
				<div class='col-lg-8' >
					...
					function noPageSelected(){
						$viewUpdates = viewPOST('WHERE post_type="Updates" Order By post_id DESC  LIMIT 0,6');
				...
				<div class='col-lg-4 ps-4 pe-4' >
					<div class="card small mb-3">
						...
						<div class="card-body">
							...
							<?php
							$viewListUpdates = viewPOST('WHERE post_type="Updates" Order By post_id DESC LIMIT '.$offset .',10');
							if(count($viewListUpdates) > 0){
								foreach($viewListUpdates as $listUpdateKEY => $listUpdate){
									...
									echo"<a href='main-updates?page=".$pageContent."&content=".$listID."' class='".$listActive." ps-2 pt-1 pe-2 pb-1'>".$viewListUpdates[$listUpdateKEY]['post_title']."</a>";
								}
							}else{
								echo'<label class="small text-left" >No Update</label>';
							}
							...
							if(count($viewListUpdates) > 0){
								...
								$result = mysqli_query(connect(), "SELECT count(*) as total FROM tbl_post WHERE post_type='Updates' ");
								...
								function pageTag($i, $active,$page,$conID){
									...
									return '<a href="main-updates?page='.$page.'&content='.$conID.'" ><span class="'.$activeStat.' ps-3 pt-2 pe-3 pb-2" >'.$i.'</span></a>';
								}
								...
								if($totalPageNumber > 0){
									...
									if(0 == $pageContent){
										...
										echo '<a href="main-updates?page='.($pageContent - 1).'&content='.$contentID.'" ><span class="ps-3 pt-2 pe-3 pb-2" >&laquo;</span></a>';
										if($totalPageNumber != 1){
									...
									if($totalPageNumber == ($pageContent + 1)){
										...
										echo '<a href="main-updates?page='.($pageContent + 1).'&content='.$contentID.'" style="" ><span class="ps-3 pt-2 pe-3 pb-2" >&raquo;</span></a>';
									}
								}
								echo"</div>";
							}
