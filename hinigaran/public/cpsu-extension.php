...
?>
		...
		<div class="container news-section pb-3">
			<div class='section-title' >
				<h1>Extension</h1>
				<p>Explore the activities of CPSU outside the campus</p>
			</div>
				...
				<div class='col-lg-8' >
					...
					function noPageSelected(){
						...
						echo "<div class='col-lg-12 mb-5'>
								<h2 class='mt-3' >Vission</h2>
								<p style='text-indent:50px;text-align: justify;text-justify: inter-word;' >To provide extension services that will bring about socio-economic growth and development for the improvement of the quality of life of the people in the hinterlands and uplands.</p>
								<img src='assets/img/extension-org-chart.jpg' class='w-100' >
							</div>
							
							<span class='h4 border-success border-top pt-3 mt-3 mb-4' >Extension Activities</span>";
									
						$viewAnnouncement = viewPOST('WHERE post_type="Extensions" Order By post_id DESC  LIMIT 0,6');
						if(count($viewAnnouncement) > 0){
							foreach($viewAnnouncement as $announcementKey => $updates){
								...
								echo "<div class='col-lg-4 mb-2 small'>
										<div class='card h-100 w-100' style='width: 18rem;'>
											...
											<div class='overlay-more'>
												<a href='main-extension?page=0&content=".$viewAnnouncement[$announcementKey]['post_id']."' class='text'>Read</a>
											</div>
										</div>
									</div>";
							}
						}else{
							echo'<label class="small text-left" >No Update</label>';
						}
					}
				...
				<div class='col-lg-4 ps-4 pe-4' >
					<div class="card small mb-3">
						...
						<div class="card-body">
							...
							<?php
							$viewListUpdates = viewPOST('WHERE post_type="Extensions" Order By post_id DESC LIMIT '.$offset .',10');
							if(count($viewListUpdates) > 0){
								foreach($viewListUpdates as $listUpdateKEY => $listUpdate){
									...
									echo"<a href='main-extension?page=".$pageContent."&content=".$listID."' class='".$listActive." ps-2 pt-1 pe-2 pb-1'>".$viewListUpdates[$listUpdateKEY]['post_title']."</a>";
								}
							}else{
								echo'<label class="small text-left" >No Update</label>';
							}
							...
							if(count($viewListUpdates) > 0){
								...
								$result = mysqli_query(connect(), "SELECT count(*) as total FROM tbl_post WHERE post_type='Extensions' ");
								...
								function pageTag($i, $active,$page,$conID){
									...
									return '<a href="main-extension?page='.$page.'&content='.$conID.'" ><span class="'.$activeStat.' ps-3 pt-2 pe-3 pb-2" >'.$i.'</span></a>';
								}
								...
								if($totalPageNumber > 0){
									...
									if(0 == $pageContent){
										...
										echo '<a href="main-extension?page='.($pageContent - 1).'&content='.$contentID.'" ><span class="ps-3 pt-2 pe-3 pb-2" >&laquo;</span></a>';
										if($totalPageNumber != 1){
									...
									if($totalPageNumber == ($pageContent + 1)){
										...
										echo '<a href="main-extension?page='.($pageContent + 1).'&content='.$contentID.'" style="" ><span class="ps-3 pt-2 pe-3 pb-2" >&raquo;</span></a>';
									}
								}
								echo"</div>";
							}
