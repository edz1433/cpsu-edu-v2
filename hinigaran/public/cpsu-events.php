		...
		<div class="container news-section pb-3">
			...
			<div class='row' >
				<div class='col-lg-8' >
								...
								foreach($viewSingleUpdates as $singleUpdateKEY => $singleUpdate){
									...
									echo "<div class='col-lg-12 border-5 border-top border-success p-3'>
											...
											<div class='d-flex justify-content-between pb-3'>
												<label class='text-muted small' ><i class='far fa-calendar-alt' ></i> ".date('M d, Y', $viewSingleUpdates[$singleUpdateKEY]['post_start'])."</label>
					...
					function noPageSelected($nowStrDate){
						$viewEvent = viewPOST('WHERE post_type="Events" AND post_start >= "'.$nowStrDate.'"  LIMIT 0,6');
						if(count($viewEvent) > 0){
							foreach($viewEvent as $eventKey => $updates){
								
								$doc = new DOMDocument();
								$postUpdateCon = $viewEvent[$eventKey]['post_content'];
								...
								echo "<div class='col-md-6 col-12 mb-2'>
										<div class='card h-100 w-100' style='width: 18rem;'>
											<img src='".$postEventImgSRC."' class='card-img-top' alt='...'>
										  
											<div class='card-body'>
												<p class='card-text fw-bold text-uppercase'>".$viewEvent[$eventKey]['post_title']."</p>
												<p class='card-text small'><i class='far fa-calendar-alt' ></i> ".date('M d, Y', $viewEvent[$eventKey]['post_start'])."</p>
											</div>
											
											<div class='overlay-more'>
												<a href='main-events?page=0&content=".$viewEvent[$eventKey]['post_id']."' class='text'>Read</a>
											</div>
										</div>
									</div>";
							}
						}else{
							echo'<label class="small" >No Event</label>';
						}
					}
				...
				<div class='col-lg-4 ps-4 pe-4' >
					<div class="card small mb-3">
						...
						<div class="card-body">
							...
							<?php
							$viewListEvents = viewPOST('WHERE post_type="Events" AND post_start >= "'.$nowStrDate.'"  LIMIT '.$offset .',10');
							if(count($viewListEvents) > 0){
								foreach($viewListEvents as $listEventKEY => $listEvent){
									$listID = $viewListEvents[$listEventKEY]['post_id'];
									if($listID == $contentID){
										$listActive = 'bg-success text-white';
									}else{
										$listActive = '';
									}
									echo"<a href='main-events?page=".$pageContent."&content=".$listID."' class='".$listActive." ps-2 pt-1 pe-2 pb-1'>".$viewListEvents[$listEventKEY]['post_title']."</a>";
								}
							}else{
								echo'<label class="small" >No Event</label>';
							}
							...
							if(count($viewListEvents) > 0){
								...
								$result = mysqli_query(connect(), "SELECT count(*) as total FROM tbl_post WHERE post_type='Events' AND post_start>='".$nowStrDate."' ");
								...
								function pageTag($i, $active,$page,$conID){
									...
									return '<a href="main-events?page='.$page.'&content='.$conID.'" ><span class="'.$activeStat.' ps-3 pt-2 pe-3 pb-2" >'.$i.'</span></a>';
								}
								...
								if($totalPageNumber > 0){
									...
									if(0 == $pageContent){
										...
										echo '<a href="main-events?page='.($pageContent - 1).'&content='.$contentID.'" ><span class="ps-3 pt-2 pe-3 pb-2" >&laquo;</span></a>';
										if($totalPageNumber != 1){
									...
									if($totalPageNumber == ($pageContent + 1)){
										...
										echo '<a href="main-events?page='.($pageContent + 1).'&content='.$contentID.'" style="" ><span class="ps-3 pt-2 pe-3 pb-2" >&raquo;</span></a>';
									}
								}
								echo"</div>";
							}
